...
func (this *EmailController) Save() {
	...
	if validate.Validate(host, is.Host) != nil {
		this.jsonError("邮件服务器主机格式不正确！")
	}
	if port == "" {
		this.jsonError("邮件服务器端口不能为空！")
	}
	if validate.Validate(port, is.Port) != nil {
...
func (this *EmailController) Test() {
	...
	emailTemplate := web.BConfig.WebConfig.ViewsPath + "/system/email/template_test.html"
